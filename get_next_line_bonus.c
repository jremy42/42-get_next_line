...
/*   Updated: 2021/11/29 09:01:16 by jremy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*memory[256] = {NULL};

	if (fd < 0 || fd > 256 || BUFFER_SIZE <= 0)
		return (NULL);
	memory[fd] = ft_read_file(fd, memory[fd]);
	if (ft_strlen(memory[fd]) > 0)
	{
		line = ft_get_line(memory[fd]);
		memory[fd] = ft_save_memory(memory[fd]);
		return (line);
	}
	return (NULL);
}
